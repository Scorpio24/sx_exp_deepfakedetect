...
from SRM.srm_filter_kernel import msca_srm
...
class HPF_3(nn.Module):
    def __init__(self):
        super(HPF_3, self).__init__()

        # Load 3 SRM Filters
        all_hpf_list_5x5 = []

        for hpf_item in msca_srm:
            if hpf_item.shape[0] == 3:
                hpf_item = np.pad(hpf_item, pad_width=((1, 1), (1, 1)), mode='constant')

            all_hpf_list_5x5.append(hpf_item)

        all_hpf_list_5x5 = np.array(all_hpf_list_5x5).reshape(3, 1, 1, 5, 5)
        all_hpf_list_5x5 = np.concatenate([all_hpf_list_5x5] * 3, axis=1)

        hpf_weight = nn.Parameter(torch.Tensor(all_hpf_list_5x5).view(3, 3, 1, 5, 5), requires_grad=False)

        self.hpf = nn.Conv3d(3, 3, kernel_size=(1, 5, 5), padding=(0, 2, 2), bias=False)
        self.hpf.weight = hpf_weight
